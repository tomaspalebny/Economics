...
#
# MODEL v2 – OPRAVENÝ:
# Pojistné je rozděleno na SOCIÁLNÍ (důchodové, nemocenské, zaměstnanost)
# a ZDRAVOTNÍ (jde do zdravotních pojišťoven, NE do státního rozpočtu).
# Pouze sociální část × sr_share_social vstupuje do příjmů SR.
# sr_share_social < 100% zohledňuje stropy, OSVČ, výjimky.
# ============================================================
...
"🇨🇿 Česká republika": dict(
    ...
    # Sazby (zákonné) – CELKOVÉ (sociální + zdravotní)
    income_tax_rate=15.0,
    employer_social=33.8, employee_social=11.0,
    # Z toho ZDRAVOTNÍ pojištění (jde do ZP, ne do SR)
    employer_health=9.0, employee_health=4.5,
    # → Do SR jde jen sociální: zaměstnavatel 24.8%, zaměstnanec 6.5%
    corporate_tax=21.0,
    vat_high=21.0, vat_low=12.0, vat_high_share=65.0,
    # Koeficienty: kolik z výnosu SOCIÁLNÍHO pojistného jde do SR
    # <100% kvůli stropům, OSVČ, výjimkám (~85% efektivní výběr)
    sr_share_social=85.0, sr_share_dpfo=60.0, sr_share_dppo=75.0,
    vat_sr_pct_gdp=5.3,
    excise_pct=2.2,
...
"🇸🇰 Slovensko": dict(
    ...
    avg_wage_month=1600,
    income_tax_rate=19.0,
    employer_social=35.2, employee_social=13.4,
    employer_health=10.0, employee_health=4.0,
    corporate_tax=21.0,
    vat_high=23.0, vat_low=10.0, vat_high_share=70.0,
    sr_share_social=85.0, sr_share_dpfo=65.0, sr_share_dppo=80.0,
    vat_sr_pct_gdp=6.8,
    excise_pct=2.5,
...
"🇩🇪 Německo": dict(
    ...
    employer_social=20.8, employee_social=20.3,
    # DE: zdravotní ~7.3% + ošetřovné ~1.7% (employer), ~8.15% + ~1.7% (employee)
    employer_health=9.0, employee_health=9.85,
    corporate_tax=30.0,
    vat_high=19.0, vat_low=7.0, vat_high_share=75.0,
    # DE: Sozialversicherung je MIMO Bundeshaushalt → sr_share_social=0
    # Bund hradí dotace do sociálních fondů přes výdajovou stranu
    sr_share_social=0.0, sr_share_dpfo=42.5, sr_share_dppo=50.0,
    vat_sr_pct_gdp=3.2,
    excise_pct=1.5,
...
"🇺🇸 USA": dict(
    ...
    avg_wage_month=5600,
    income_tax_rate=22.0,
    employer_social=7.65, employee_social=7.65,
    # USA: Medicare 1.45% je „zdravotní", ale jde do federálního trust fund
    # → počítáme vše jako federální příjem
    employer_health=0.0, employee_health=0.0,
    corporate_tax=21.0,
...
"🇷🇺 Rusko": dict(
    ...
    employer_social=30.0, employee_social=0.0,
    # RU: zdravotní (ФОМС) 5.1% z 30% zaměstnavatele
    employer_health=5.1, employee_health=0.0,
    corporate_tax=25.0,
    vat_high=20.0, vat_low=10.0, vat_high_share=70.0,
    # RU: sociální pojistné jde do mimorozpočtových fondů (ПФР, ФСС, ФОМС)
...
"⚙️ Vlastní": dict(
    ...
    employer_social=25.0, employee_social=10.0,
    employer_health=8.0, employee_health=4.0,
    corporate_tax=20.0,
...
employer_social = st.sidebar.number_input("Pojistné – zaměstnavatel (celkem)", min_value=0.0, max_value=60.0, value=float(p["employer_social"]), step=0.5, format="%.1f")
employee_social = st.sidebar.number_input("Pojistné – zaměstnanec (celkem)", min_value=0.0, max_value=40.0, value=float(p["employee_social"]), step=0.5, format="%.1f")

st.sidebar.markdown("---")
st.sidebar.header("🏥 Z toho zdravotní pojištění (%)")
with st.sidebar.expander("ℹ️ Proč odděleně?"):
    st.markdown("""
Zdravotní pojištění **nejde do státního rozpočtu** – jde přímo
zdravotním pojišťovnám (VZP, ČPZP apod.). Model proto odečítá
zdravotní část a do příjmů SR počítá jen **sociální pojištění**
(důchodové, nemocenské, příspěvek na politiku zaměstnanosti).

**ČR 2024:** zaměstnavatel 9.0%, zaměstnanec 4.5%
**SK:** zaměstnavatel 10.0%, zaměstnanec 4.0%
**DE/RU:** sociální pojistné jde do mimorozpočtových fondů
""")
employer_health = st.sidebar.number_input("Zdravotní – zaměstnavatel", min_value=0.0, max_value=20.0, value=float(p["employer_health"]), step=0.5, format="%.1f")
employee_health = st.sidebar.number_input("Zdravotní – zaměstnanec", min_value=0.0, max_value=20.0, value=float(p["employee_health"]), step=0.5, format="%.1f")

# Odvozené sociální sazby (bez zdravotního)
employer_social_only = max(employer_social - employer_health, 0.0)
employee_social_only = max(employee_social - employee_health, 0.0)
st.sidebar.info(f"→ Sociální do SR: zaměstnavatel {employer_social_only:.1f}%, zaměstnanec {employee_social_only:.1f}%")
...
- **ČR**: Soc. pojistné ~85% (stropy, OSVČ), DPFO ~60%, DPPO ~75%
- **DE**: Soc. pojistné 0% (jde do Sozialversicherung), DPFO 42.5%, DPPO 50%
- **USA**: Vše 100% (federální daně)
- **RU**: Soc. pojistné 0% (mimorozpočtové fondy), DPFO 0% (regionální daň)
""")
sr_share_social = st.sidebar.number_input("Soc. pojistné → SR (%)", min_value=0.0, max_value=100.0, value=float(p["sr_share_social"]), step=1.0, format="%.0f")
...
# Výnos SOCIÁLNÍHO pojistného (BEZ zdravotního) → státní rozpočet
social_rate_sr = employer_social_only + employee_social_only
total_social_sr_base = total_wage_bill * social_rate_sr / 100
social_sr = total_social_sr_base * sr_share_social / 100

# Zdravotní pojistné (informativně – nejde do SR)
health_rate_total = employer_health + employee_health
health_total = total_wage_bill * health_rate_total / 100

# DPFO – z efektivního mzdového fondu (slevy, odpočty → ~55% efektivní sazba)
dpfo_eff_rate = income_tax * 0.55
dpfo_full = total_wage_bill * dpfo_eff_rate / 100
dpfo_sr = dpfo_full * sr_share_dpfo / 100

# DPPO – z HDP přes ziskovou marži
corp_profit_base = gdp * 0.20
...
# Informativní box o zdravotním pojištění
st.info(f"ℹ️ **Zdravotní pojištění** (mimo SR): {health_total:,.0f} {cur} "
        f"(sazba {health_rate_total:.1f}% z mezd → zdravotní pojišťovny)")
...
st.markdown("---")
st.caption(f"""⚠️ Výukový model v2 – zjednodušená aproximace STÁTNÍHO ROZPOČTU (centrální vláda).
